...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.$nextTick(()=>{
        let thdom = document.getElementsByClassName('.el-table th')
        console.log(thdom)
      })
    },
    watch: {
    ...
    methods: {
      ...
      rowDelScope(row) {
        this.$confirm("确定将选择数据删除?", {
          ...
          return removeDataScope(row.id).then(res => {
            if (res.data.code != 200){
              let that = this;
              errorMsgbox(that, res.data.msg);
            } else {
      ...
      handleDeleteScope() {
        ...
        this.$confirm("确定将选择数据删除?", {
          ...
          removeDataScope(this.scopeIds).then(res => {
            if (res.data.code != 200){
              let that = this;
              errorMsgbox(that, res.data.msg);
            } else {
