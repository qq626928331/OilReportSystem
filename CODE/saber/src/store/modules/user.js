...
const user = {
  state: {
    ...
    menuAll: getStore({name: 'menuAll'}) || JSON.parse(localStorage.getItem('menuAll')) || [],
    // menuAll: getStore({name: 'menuAll'}) || [],
    token: getStore({name: 'token'}) || '',
    refreshToken: getStore({name: 'refreshToken'}) || '',
    keepAliveExcludeMenus: [],
  },
  actions: {
    //根据用户名登录
    ...
    GetMenu({commit, dispatch}, topMenuId) {
      return new Promise(resolve => {
        if (topMenuId!=''&&topMenuId!=undefined&&topMenuId!=null){
          getRoutes(topMenuId).then((res) => {
            ...
            let menuAll = user.state.menuAll;
            menu.forEach(item => {
              menuAll.push(item);
            })
  ...
  mutations: {
    SET_KEEP_ALIVE_EXCLUDE_MENUS: (state, keepAliveExcludeMenus) => {
      state.keepAliveExcludeMenus = keepAliveExcludeMenus;
    },
    ...
    SET_MENU_ALL: (state, menuAll) => {
      ...
      localStorage.setItem("menuAll", JSON.stringify(menuAll));//wqq
